...
- [Comprendre ArangoSearch](getting-started/arangosearch.md) — l'introduction complète au moteur de recherche : Analyzers, Views, `SEARCH`, scoring, ce qu'ils rendent possible, et comment chaque étage de la bibliothèque s'y rattache.
...
- [Fonctions numériques](aql/aql-functions-numerics.md) — 35 fonctions AQL côté nombres (dont distances vectorielles).
...
- [Champs conditionnels `Field::WHEN`](db/conditional-fields.md) — garder une valeur projetée derrière une condition (`cond ? valeur : else`), groupes AND/OR/NOT, `alt` sur les opérandes.
...
- [Signaux & cascade](lifecycle-signals-cascade.md) — les 6 signaux du cycle de vie (`before*`/`after*`), la cascade de suppression (purge automatique des arêtes + purge dirigée des documents liés `Purge::INBOUND`/`OUTBOUND`/`BOTH`) et l'invalidation des caches dépendants sur écriture (`Arango::INVALIDATES`).
- [La projection des champs](projection.md) — `Field::SKINS`, `AQL::SKIN_FIELDS`, `AQL::REQUIRES`, `Field::ALTERS`, authorizer câblé automatiquement par la base.
- [Projection des edges et joins](edges-joins-projection.md) — `AQL::EDGES`, `AQL::JOINS`, traversées hiérarchiques, `Filter::WRAP`, `AQL::SKIN` (cycles).
...
- [Commande de maintenance `command:arangodb`](commands/arangodb.md) — `dump` / `restore` / `collections` / `views` / `doctor` / `migrate`, et la page [Stratégies de dump/restore](commands/dump-restore-strategies.md) (recettes prêtes à l'emploi).
