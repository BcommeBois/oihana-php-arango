...
|---|---|---|
| Barre de recherche avec pertinence | match `TOKENS` + `BM25` | [bloc `AQL::VIEW`](../db/search/overview.md), automatique |
...
| Bloc `AQL::VIEW` du modèle | `?search=` bascule sur la View, auto-provisioning, clé de tri `score`, totaux & facet counts synchronisés | [Recherche View](../db/search/overview.md) |
...
- **Recherche fine i18n** au-delà du `?lang=` déjà disponible ([Analyzer par champ](../db/search/per-field-options.md#analyzer-par-champ) et [recherche localisée `?lang=`](../db/search/per-field-options.md#recherche-localisée-lang)) : par exemple un même champ indexé avec plusieurs Analyzers.
...
Le gating de la recherche par permissions est désormais [disponible](../db/search/per-field-options.md#permissions-de-recherche).
...
- [Recherche View (ArangoSearch)](../db/search/overview.md) — la recette modèle (`AQL::VIEW`, URLs, réponses JSON).
...
- [Recherche `?search=`](../db/search/README.md) — le balayage `LIKE` simple, toujours le bon outil pour les petits modèles.
