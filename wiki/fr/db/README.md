...
| 8b | [Bornes HTTP `?bounds=`](bounds.md) | L'étendue `{ min, max }` d'un champ numérique à côté de la liste : déclaration `AQL::BOUNDS` (liste blanche fermée par défaut, champ plat / mesure imbriquée `[*]`), filtre partagé, gate `REQUIRES` anti-oracle, et le drapeau générique `?metaOnly=` (barre latérale sans documents). |
