...
| 5b | [Recherche View (ArangoSearch)](search/README.md) | La déclaration `AQL::VIEW` : `?search=` bascule vers une recherche accélérée par index et **classée par pertinence** (boosts, bonus phrase, fuzzy, clé de tri `score`, View auto-provisionnée). |
...
| 5d | [Recherche fédérée multi-collections](federated-search.md) | Une **barre unique** sur plusieurs collections (`FederatedSearch`) : approche « trouver puis reconstruire », pagination + total, skin, et le **filtre de permission par collection** (avec exemples). |
