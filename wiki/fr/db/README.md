...
| 5 | [Recherche HTTP `?search=`](search/README.md) | Recherche multi-champs `LIKE` (insensible à la casse), déclaration `searchable`, combinaison, limites (vs ArangoSearch). |
| 5b | [Recherche View (ArangoSearch)](search/overview.md) | La déclaration `AQL::VIEW` : `?search=` bascule vers une recherche accélérée par index et **classée par pertinence** (boosts, bonus phrase, fuzzy, clé de tri `score`, View auto-provisionnée). |
...
| 5d | [Recherche fédérée multi-collections](search/federated.md) | Une **barre unique** sur plusieurs collections (`FederatedSearch`) : approche « trouver puis reconstruire », pagination + total, skin, et le **filtre de permission par collection** (avec exemples). |
...
| 8b | [Bornes HTTP `?bounds=`](bounds.md) | L'étendue `{ min, max }` d'un champ numérique à côté de la liste : déclaration `AQL::BOUNDS` (liste blanche fermée par défaut, champ plat / mesure imbriquée `[*]`), filtre partagé, contrôle d'autorisation `REQUIRES` anti-oracle, et le drapeau générique `?metaOnly=` (barre latérale sans documents). |
...
| 10 | [Tri `?sort=` / `?near=`](sort.md) | La *whitelist* fail-closed `AQL::SORTABLE` (3 notations), `SORT_DEFAULT`, le **contrôle d'autorisation** du tri (hérité de `$fields` ou explicite — pas d'oracle de tri), les clés synthétiques `distance` / `score`, et le **tri par distance `?near=`** (clé géo whitelistée, `GeoIndex`). |
