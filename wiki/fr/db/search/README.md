...
## Toute la recherche en un coup d'œil

Le même paramètre `?search=` couvre toute une gamme, de la barre simple au moteur fédéré — **sans changement d'URL** côté client. Choisissez selon le besoin :

| Niveau | Page | Pour quoi |
|---|---|---|
| **Défaut** | recherche simple (cette page, ci-dessous) | `LIKE` multi-champs, insensible à la casse, zéro config. Suffisant au quotidien. |
| Pertinence | [Recherche View (ArangoSearch)](overview.md) | Index inversé + score `BM25`, boosts, phrase, fuzzy. **Le full-text.** |
| Par champ | [Options par champ](per-field-options.md) | Boost, fuzzy, Analyzer, autocomplétion, langue, phrase, permissions — champ par champ. |
| Tableaux | [Champs de tableaux d'objets](array-fields.md) | Sous-champ d'un tableau d'objets (`contactPoints[*].email`). |
| Multi-collections | [Recherche fédérée](federated.md) | Une barre unique sur plusieurs collections, classée par pertinence. |
| Préparation texte | [Analyzers](../analyzers.md) | Tokenisation, stemming, accents, n-grammes — la brique sous la View. |

La suite de cette page documente le **niveau par défaut** (le balayage `LIKE`).
