...
| 8b | [HTTP bounds `?bounds=`](bounds.md) | The `{ min, max }` extent of a numeric field beside the list: `AQL::BOUNDS` declaration (fail-closed whitelist, flat field / nested `[*]` measure), shared filter, anti-oracle `REQUIRES` gate, and the generic `?metaOnly=` flag (sidebar without documents). |
