...
> **Beyond filtering.** A list query also has **sorting** ([`?sort=` / `?near=`](sort.md)), **pagination** (`?limit`/`?offset`) and output **projection** (skins, `Field::*`). These do not *filter* and are out of scope here — see [Sorting](sort.md) and [Field projection](../projection.md).
...
Distance sorting is covered with the rest of sorting on its dedicated page: **[Sorting → Distance sorting (`?near=`)](sort.md#distance-sorting-near)** (anchor point, `distance` key, geo-key whitelisting, `?near=`/`?sort=` combinations, `GeoIndex`).
...
- [Sorting `?sort=` / `?near=`](sort.md) — the `SORTABLE` whitelist, sort permission, distance sorting.
