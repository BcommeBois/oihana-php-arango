...
|---|---|---|
| `aqlFieldDefault` | `Filter::DEFAULT` | Simple reference `key: doc.keyName`. Also backs `Filter::ID`, whose default source is `_key` — the document key projected under a public name, **as stored**: it is a string, never converted. |
...
| `aqlFieldNumber` | `Filter::NUMBER` | Numeric cast `key: TO_NUMBER(doc.x)`, with an optional `Field::NULLABLE` guard so a missing attribute yields `null` instead of `0`. |
...
| `Field::NULLABLE` | On a `Filter::DOCUMENT`: yield `null` when the source is missing, instead of an object rebuilt out of nothing. See [Guarding a sub-document](conditional-fields.md#guarding-a-sub-document--fieldnullable). | `thing:IS_OBJECT(doc.thing) ? { … } : null` |
...
Two functions tied to the *skin* projection system (covered in detail in [Field projection](../projection.md)):
...
- [Field projection](../projection.md) — `matchesSkin` and `resolveSkinFields` in context.
