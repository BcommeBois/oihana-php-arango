 ...
 *
 * - {@see Field::NULLABLE} — the declared intent « nothing to build from, no value »,
 *   compiled by default to an `IS_OBJECT()` test on `$source`. On a rebuilt object that is
 *   deliberately a **type** test and not a `!= null` comparison: an attribute that exists
 *   but is not an object (a string, a number) rebuilds the very same object of nulls, and
 *   the house style tests the type ({@see aqlFieldArray()}, {@see aqlFieldObject()}).
 *   A caller whose shape calls for another test passes it as `$test` — {@see aqlFieldBool()}
 *   asks for `!= null`, because `IS_BOOL()` would make every document storing `1` or
 *   `"yes"` abstain, when `TO_BOOL()` exists precisely to accept them.
 ...
 * @param string|null            $test    The condition `Field::NULLABLE` compiles to. Defaults to
 *                                        `IS_OBJECT($source)`, the test a rebuilt object calls for.
 *
 ...
 * @since   1.6.0
 */
function guardProjection( string $value , array $options , string $doc , string $source , ?string $test = null ) : string
{
    $conditions = [] ;

    if ( ( $options[ Field::NULLABLE ] ?? false ) === true )
    {
        $conditions[] = $test ?? isObject( $source ) ;
    }
