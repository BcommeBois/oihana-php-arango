 ...
 *
 * Mirrors the `arangodump` `datetime` masker: a random instant is picked in
 * `[begin, end]` and rendered with `format`. The format uses the AQL
 * `DATE_FORMAT()` tokens — the common ones are supported: `%yyyy`, `%yy`,
 * `%mm`, `%m`, `%dd`, `%d`, `%hh`, `%h`, `%ii`, `%i`, `%ss`, `%s`, `%fff` and
 * `%%`. When `format` is empty (the default), an **empty string** is returned,
 * matching the binary's behavior.
 *
 * @example
 * ```php
 ...
 * @param string $format The AQL DATE_FORMAT pattern. Empty returns "".
 * @return string
 *
